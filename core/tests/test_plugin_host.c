...
static void test_register_and_run(void) {
    ...
    assert(host != NULL);
    assert(ase_plugin_host_command_count(host) == 0);

    assert(ase_plugin_host_register_command(host, "noop", noop_command, NULL));
    assert(ase_plugin_host_command_count(host) == 1);

    AseBuffer *buf = ase_buffer_create();
    assert(ase_plugin_host_run_command(host, "noop", buf));
    assert(!ase_plugin_host_run_command(host, "does_not_exist", buf));
...
static void test_load_directory_missing_is_not_an_error(void) {
    AsePluginHost *host = ase_plugin_host_create();
    assert(host != NULL);
    assert(ase_plugin_host_load_directory(host, "this_directory_does_not_exist") == 0);
    assert(ase_plugin_host_command_count(host) == 0);
    ase_plugin_host_destroy(host);
}
...
static void test_lua_and_native_plugins_from_directory(void) {
    ...
    assert(loaded == 2); /* uppercase.lua + the compiled test_native_plugin */
    assert(ase_plugin_host_command_count(host) == 2);

    AseBuffer *buf = ase_buffer_create();
    assert(ase_buffer_insert(buf, 0, "hello", 5));

    assert(ase_plugin_host_run_command(host, "lua_uppercase", buf));
    char out[16];
    size_t n = ase_buffer_get_text(buf, 0, ase_buffer_length(buf), out);
    assert(n == 5 && memcmp(out, "HELLO", 5) == 0);

    assert(ase_plugin_host_run_command(host, "native_reverse", buf));
    n = ase_buffer_get_text(buf, 0, ase_buffer_length(buf), out);
    assert(n == 5 && memcmp(out, "OLLEH", 5) == 0);
