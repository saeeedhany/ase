...
static void test_parse_primitives(void) {
    ...
    v = parse("null");
    CHECK(v != NULL && ase_json_type(v) == ASE_JSON_NULL);
    ase_json_destroy(v);

    v = parse("true");
    CHECK(v != NULL && ase_json_get_bool(v, false) == true);
    ase_json_destroy(v);

    v = parse("false");
    CHECK(v != NULL && ase_json_get_bool(v, true) == false);
    ase_json_destroy(v);

    v = parse("42");
    CHECK(v != NULL && ase_json_get_number(v, -1) == 42.0);
    ase_json_destroy(v);

    v = parse("-3.5");
    CHECK(v != NULL && ase_json_get_number(v, 0) == -3.5);
    ase_json_destroy(v);

    v = parse("1.5e2");
    CHECK(v != NULL && ase_json_get_number(v, 0) == 150.0);
    ase_json_destroy(v);

    v = parse("\"hello\"");
    CHECK(v != NULL && strcmp(ase_json_get_string(v), "hello") == 0);
    ase_json_destroy(v);
}
...
static void test_whitespace_tolerance(void) {
    ...
    CHECK(v != NULL);
    CHECK(ase_json_get_number(ase_json_object_get(v, "a"), -1) == 1.0);
    ase_json_destroy(v);
}
...
static void test_string_escapes(void) {
    ...
    CHECK(v != NULL);
    CHECK(strcmp(ase_json_get_string(v), "a\"b\\c\nd\te") == 0);
    ase_json_destroy(v);
}
...
static void test_unicode_escapes(void) {
    ...
    CHECK(strlen(s) == 5); /* c a f + 2-byte utf8 */
...
static void test_surrogate_pair(void) {
    ...
    const char *s = ase_json_get_string(v);
    CHECK(strlen(s) == 4);
    CHECK((unsigned char)s[0] == 0xF0 && (unsigned char)s[1] == 0x9F);
    CHECK((unsigned char)s[2] == 0x98 && (unsigned char)s[3] == 0x80);
    ase_json_destroy(v);
}
...
static void test_nested_structure(void) {
    ...
    CHECK(v != NULL);
    CHECK(ase_json_type(v) == ASE_JSON_OBJECT);
    CHECK(strcmp(ase_json_get_string(ase_json_object_get(v, "jsonrpc")), "2.0") == 0);
    CHECK(ase_json_get_number(ase_json_object_get(v, "id"), -1) == 7.0);

    AseJsonValue *result = ase_json_object_get(v, "result");
    CHECK(result != NULL);
    AseJsonValue *items = ase_json_object_get(result, "items");
    CHECK(ase_json_array_size(items) == 3);
    CHECK(ase_json_get_number(ase_json_array_get(items, 0), -1) == 1.0);
    CHECK(ase_json_get_number(ase_json_array_get(items, 2), -1) == 3.0);
    CHECK(ase_json_get_bool(ase_json_object_get(result, "ok"), false) == true);
    CHECK(ase_json_type(ase_json_object_get(result, "note")) == ASE_JSON_NULL);

    CHECK(ase_json_object_get(v, "no_such_key") == NULL);
    CHECK(ase_json_array_get(items, 99) == NULL);
...
static void test_builder_and_write_round_trip(void) {
    ...
    CHECK(ase_json_object_set(obj, "name", ase_json_string("ase")));
    CHECK(ase_json_object_set(obj, "version", ase_json_number(1)));
    CHECK(ase_json_object_set(obj, "ok", ase_json_bool(true)));
    CHECK(ase_json_object_set(obj, "extra", ase_json_null()));

    AseJsonValue *arr = ase_json_array();
    CHECK(ase_json_array_append(arr, ase_json_number(1)));
    CHECK(ase_json_array_append(arr, ase_json_number(2)));
    CHECK(ase_json_object_set(obj, "items", arr));

    /* replacing an existing key must not grow the object or leak the old value */
    CHECK(ase_json_object_set(obj, "version", ase_json_number(2)));
    ...
    CHECK(reparsed != NULL);
    CHECK(strcmp(ase_json_get_string(ase_json_object_get(reparsed, "name")), "ase") == 0);
    CHECK(ase_json_get_number(ase_json_object_get(reparsed, "version"), -1) == 2.0);
    CHECK(ase_json_get_bool(ase_json_object_get(reparsed, "ok"), false) == true);
    CHECK(ase_json_array_size(ase_json_object_get(reparsed, "items")) == 2);
...
static void test_write_escapes_control_and_quotes(void) {
    ...
    AseJsonValue *reparsed = ase_json_parse(text, strlen(text));
    CHECK(reparsed != NULL);
    CHECK(strcmp(ase_json_get_string(reparsed), "line1\nline2\t\"quoted\"\\backslash") == 0);
...
static void test_ownership_on_failed_append(void) {
    ...
    CHECK(!ase_json_array_append(not_an_array, ase_json_number(2)));

    AseJsonValue *not_an_object = ase_json_number(1);
    CHECK(!ase_json_object_set(not_an_object, "k", ase_json_number(2)));
