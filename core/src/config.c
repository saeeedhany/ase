...
static const char kDefaultConfigTemplate[] =
    ...
    "#\n"
    "# Two chords in a row use `>`: `key.ctrl+w>j` is Ctrl+W then j. The\n"
    "# built-in ones live under Ctrl+W, which is Vim's window prefix, and\n"
    "# press it alone to see what it accepts. The layers the defaults\n"
    "# follow, if you are adding your own: Ctrl for what you brought from\n"
    "# other editors, Ctrl+W for regions, Alt for this editor's own\n"
    "# panels and pickers, F-keys for the language server, and bare keys\n"
    "# left entirely to Vim. See docs/adr/0120.\n"
    "# key.ctrl+s = editor.save\n"
    "# key.f5 = editor.compile\n"
    "# key.ctrl+w>t = editor.open-in-project\n"
    "# key.alt+b = none\n"
    "\n"
